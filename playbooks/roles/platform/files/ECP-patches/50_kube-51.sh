...
CANAL_TEMPLATES_DIR="$K8S_DIR/canal-templates"
...
get_rpms() {
    ...
    then
        ...
        then
            ...
            K8S_RPMS="${K8S_KUBEADM}-${bds_k8s_version} kubernetes-common-$bds_k8s_version \
                      ${K8S_KUBELET}-${bds_k8s_version} ${K8S_KUBECTL}-${bds_k8s_version}"
        fi
    fi
...
setup_docker_creds_for_airgap() {
     ...
     if [ -n "$containerRepo" ] && [ -n "$repoUsername" ] && [ -n "$where" ];
     then
...
setup_root_docker_creds() {
    ...
    then
        ...
        setup_docker_creds_for_airgap $bds_k8s_containerrepo $bds_k8s_containerrepousername $root_docker_dir
    fi
}
...
setup_kubelet_docker_creds() {
    ...
    setup_docker_creds_for_airgap $bds_k8s_containerrepo $bds_k8s_containerrepousername "/var/lib/kubelet/"
}
...
setup_hpecp_plugin() {
    ...
    log_sudo_exec "curl -o /usr/bin/kubectl-hpecp -L http://$CONTROLLER/thirdparty/$KUBECTL_HPECP_BIN_PLUGIN"
...
set_kubelet_flags() {
    ...
    log_sudo_exec "sed -i 's/\"$/ --feature-gates=RotateKubeletServerCertificate=true\"/' $kubeadm_env"
}
...
bootstrap_setup_network() {
    ...
    util_copy_file "$CANAL_TEMPLATES_DIR/canal-template.yaml"  "$CANAL_TEMPLATES_DIR/canal.yaml"
    log_exec "sed -i 's|@@@@POD_NETWORK_RANGE@@@@|$bds_k8s_podnetworkrange|g' $CANAL_TEMPLATES_DIR/canal.yaml"

    # Calculate the mtu size to use
    BASE_MTU_SIZE=$(cat /sys/class/net/$bds_network_publicinterface/mtu)
    VETH_MTU_SIZE=$(( BASE_MTU_SIZE - 50 ))

    log_exec "sed -i 's|@@@@VETH_MTU@@@@|$VETH_MTU_SIZE|g' $CANAL_TEMPLATES_DIR/canal.yaml"

    insert_airgap_repo "$CANAL_TEMPLATES_DIR/canal.yaml"

    log_exec kubectl apply -f "$CANAL_TEMPLATES_DIR/canal.yaml"
}

bootstrap_wait_for_network() {
    # Wait for canal daemonsets to be up and running on master
    # This can take upto 20-25 minutes. Waiting for 30 minutes
    Retries=60
    while [ "$Retries" -gt 0 ]; do
        DS_PODS=$(kubectl -n kube-system get daemonsets canal -o jsonpath='{.status.numberReady}{" "}{.status.desiredNumberScheduled}' 2>/dev/null)
        NUM_DS_READY=$(echo $DS_PODS | cut -d" " -f1)
        NUM_DS_DESIRED=$(echo $DS_PODS | cut -d" " -f2)
        if [ "$NUM_DS_READY" == "$NUM_DS_DESIRED" ]
        then
            break
        fi

        log "Waiting for canal daemonset pods to be running numberReady $NUM_DS_READY: desiredNumber : $NUM_DS_DESIRED"
        sleep 30
        Retries=`expr $Retries - 1`
        if [ $Retries -le 0 ]; then
            log "Failed waiting for canal daemonset pods to be running."
            exit 1
        fi
    done
}
...
bootstrap_setup_ext_mapr_csi() {
    ...
    util_copy_file "$MAPR_TEMPLATES_DIR/11_csi-deploy-provisioner-template.yaml" "$MAPR_TEMPLATES_DIR/csi/11_csi-deploy-provisioner.yaml"
    insert_airgap_repo "$MAPR_TEMPLATES_DIR/csi/11_csi-deploy-provisioner.yaml"
    ...
    util_copy_file $MAPR_TEMPLATES_DIR/storageclass-template-ext.yaml $MAPR_TEMPLATES_DIR/storageclass.yaml
    ...
    log_exec "sed -i 's|@@@@EXT_MAPR_MOUNT_DIR@@@@|$EXT_MAPR_MOUNT_DIR|g' $MAPR_TEMPLATES_DIR/storageclass.yaml"
...
bootstrap_setup_mapr_csi() {
    ...
    CSI_USER=$(echo -n "$bds_k8s_clustername" | base64)
    CSI_PASS=$(cat $HOST_MAPR_CONF_DIR/${bds_k8s_clustername}-pass | base64)
    SERVICE_TICKET=$(cat $HOST_MAPR_CONF_DIR/${bds_k8s_clustername}-ticket | base64 -w 0)
    ...
    log_exec "sed -i 's|@@@@MAPR_CLDB_SERVERS@@@@|$CLDB_NODES|g' $MAPR_TEMPLATES_DIR/storageclass.yaml"
    log_exec "sed -i 's|@@@@K8S_CLUSTER_NAME@@@@|$bds_k8s_clustername|g' $MAPR_TEMPLATES_DIR/storageclass.yaml"
    log_exec "sed -i 's|@@@@K8S_VOLUME_USER@@@@|$bds_k8s_clustername|g' $MAPR_TEMPLATES_DIR/storageclass.yaml"
    log_exec "sed -i 's|@@@@MAPR_CLUSTER_NAME@@@@|$bds_storage_maprclustername|g' $MAPR_TEMPLATES_DIR/storageclass.yaml"
...
EOF
        ...
        then
            ...
            # No hyperkube image in OpenSUSE K8S repo
            ...
            [[ "${bds_global_osname}" == 'suse' ]] && useHyperKubeImage=false
...
apiServer:
  ...
  extraArgs:
    ...
    enable-admission-plugins : PodPreset,NodeRestriction,PodSecurityPolicy
...
EOF
            ...
            then
                ...
                then
                    IMAGE_REPO="${bds_k8s_containerrepo}/registry.suse.com/caasp/v4"
...
upgrade_rollback_kube() {
    ...
    # todo: Don't we have to undo the docker repo and restore the previous one?

    # todo: Don't we have to rollback yum repo?
}
